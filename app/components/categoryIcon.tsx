...
const SVG_SIZES: { default: number } & Partial<Record<Category, number>> = {
    snacks: 24,
    "sugar/desserts": 24,
    preserved: 28,
    remaining: 20,
...
const getSVGSize = (svgSizeSmaller: number | null | undefined, type: Category) => {
...
const backgroundColors: CategoriesColors = {
    ...
    dairy: { light: "#e6d0b1", dark: "#E3C9B0" },
...
export default function CategoryIcon({ category, size = 48, svgSizeSmaller = null }: Props) {
    const { theme } = useThemes()
    const safeCategory: Category = category in backgroundColors ? category : "remaining"

    switch (category) {
        case "bread":
            return (
                <Wrapper size={size} category={safeCategory} theme={theme}>
                    <Svg
                        fill="white"
                        width={getSVGSize(svgSizeSmaller, safeCategory)}
                        height={getSVGSize(svgSizeSmaller, safeCategory)}
                        viewBox="0 0 24 24"
                    >
