...
type ProductsSearchService interface {
	SearchProducts(ctx context.Context, query string, categories []string, page string) (*contracts.ProductsSearchResponse, error)
	FuzzySearchProducts(ctx context.Context, query string, category string, page string) (*contracts.ProductsSearchResponse, error)
...
func (psh *ProductsSearchHandler) FuzzySearchProducts(c echo.Context) error {
	...
	result, err := psh.ProductsSearchService.FuzzySearchProducts(c.Request().Context(), query, category, page)
	if err != nil {
		return response.Error(c, http.StatusInternalServerError, err.Error())
	}
